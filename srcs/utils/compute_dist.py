...
@torch.no_grad()
def compute_euclidean_distance(feature_1, feature_2):
    m, n = feature_1.size(0), feature_2.size(0)
    dist_m = (
            torch.pow(feature_1, 2).sum(dim=1, keepdim=True).expand(m, n)
            + torch.pow(feature_2, 2).sum(dim=1, keepdim=True).expand(n, m).t()
    )
    dist_m.addmm_(1, -2, feature_1, feature_2.t())

    return dist_m.cpu().numpy()

@torch.no_grad()
def compute_cosine_distance(feature_1, feature_2):
    ...
    feature_2 = F.normalize(feature_2, p=2, dim=2)
    dist_m = 1 - torch.mm(feature_1, feature_2.T)
    return dist_m
